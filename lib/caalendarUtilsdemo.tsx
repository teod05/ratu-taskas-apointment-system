...
// ── Calendar config ──────────────────────────────────────────────
const HOUR_START = 7;          // first visible hour (07:00)
const HOUR_END = 20;           // last visible hour  (20:00)
const TOTAL_HOURS = HOUR_END - HOUR_START;
const PX_PER_HOUR = 80;        // height of one hour row in px
...
export default function CalendarPage() {
  ...
  // ── Appointment lookup per day ───────────────────────────────────
  function appointmentsForDay(day: Date): Appointment[] {
    const key = toDateString(day);
    return appointments.filter((a) => {
      // a.date comes back as "YYYY-MM-DDT00:00:00.000Z" from Prisma
